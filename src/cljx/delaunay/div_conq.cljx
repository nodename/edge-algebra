...
(defn connect!
  [a b]
  (println "connect!: connecting" (edge-info a) "-->>" (edge-info b))
  (let [e (make-d-edge! (dest a) (org b))]
    (println "connect: first splice" (edge-info e) (edge-info (l-next a)))
    (splice! e (l-next a))
    (println "connect: second splice" (edge-info (sym e)) (edge-info b))
    (splice! (sym e) b)
    (println "o-next sym e:" (o-next (sym e)) (edge-info (o-next (sym e))))
    #_(show-edge-records)
    e))

(defn delete-edge!
  [e]
  (println "delete-edge: Splicing" (edge-info e) (edge-info (o-prev e)))
  (splice! e (o-prev e))
  (println "delete-edge: Splicing" (edge-info (sym e)) (edge-info (o-prev (sym e))))
  (splice! (sym e) (o-prev (sym e)))
  (println "removing " (edge-info e))
...
(defn right-of?
  ...
  (let [result (ccw? point (dest edge) (org edge))]
    (println "right-of?" point (edge-info edge) result)
    result))

(defn left-of?
  [point edge]
  (let [result (ccw? point (org edge) (dest edge))]
    (println "left-of?" point (edge-info edge) result)
    result))
...
(defn slide-left!
  [edge]
  (let [t (o-next edge)]
    (println "slide-left:" (edge-info t))
    (delete-edge! edge)
    t))
...
(defn slide-right!
  [edge]
  (let [t (o-prev edge)]
    (println "slide-right:" (edge-info t))
    (delete-edge! edge)
    t))
...
(defn bubble-left!
  ...
  [cross-edge]
  (println "bubble-left entering cross-edge:" cross-edge (edge-info cross-edge))
  #_(println "sym:" (sym cross-edge) (edge-info (sym cross-edge)))
  #_(println "o-next:" (o-next (sym cross-edge)) (edge-info (o-next (sym cross-edge))))

  (println "bubble-left: (sym cross-edge)'s o-next ring:")
  (show-ring o-next (sym cross-edge))
  (let [initial-edge (o-next (sym cross-edge))]
    (println "bubble-left initial edge:" (edge-info initial-edge))
    (if (dest-above? initial-edge cross-edge)
      (do  (println "bubble-left: dest is above cross-edge")
      (loop [edge initial-edge]
        (println "bubble-left: edge:" (edge-info edge))
...
(defn bubble-right!
  ...
  (let [initial-edge (o-prev cross-edge)]
    (println "bubble-right initial edge: " (edge-info initial-edge))
    (if (dest-above? initial-edge cross-edge)
      (do  (println "bubble-right: dest is above cross-edge")
      (loop [edge initial-edge]
        (println "bubble-right: edge:" (edge-info edge))
        (if (in-circle? (dest cross-edge) (org cross-edge) (dest edge)
                        (dest (o-prev edge)))
...
(defn lower-common-tangent
  ...
  ;; Or IS IT?
  (println "lower-common-tangent in: ldi:" (edge-info ldi) "rdi:" (edge-info rdi))
  (let [[ldi rdi] (cond
                   (left-of? (org rdi) ldi)  [(l-next ldi) rdi]
                   (right-of? (org ldi) rdi) [ldi (r-prev rdi)]
                   :else [ldi rdi])]
    (println "lower-common-tangent out: ldi:" (edge-info ldi) "rdi:" (edge-info rdi))
    [ldi rdi]))
...
(defn delaunay'
  ...
  (condp = (count sites)
      ...
      3 (let [[s1 s2 s3] sites
              a (make-d-edge! s1 s2)
              b (make-d-edge! s2 s3)]
          (println "triangle: Splicing" (edge-info (sym a)) (edge-info b))
          (splice! (sym a) b)
          ...
          (cond
           ...
           (ccw? s1 s3 s2) (do
                             (let [c (connect! b a)]
                               (println "triangle returning" (edge-info (sym c)) (edge-info c))
                               [(sym c) c]))
      ...
      (let [[l r] (halves sites)
            ...
            [ldo ldi] (delaunay' l)
            [rdi rdo] (delaunay' r)
            _ (println "ldo:" (edge-info ldo) "ldi:" (edge-info ldi))
            _ (println "rdo:" (edge-info rdo) "rdi:" (edge-info rdi))
            ;; Compute the lower common tangent [ldi rdi] of l and r:
            [ldi rdi] (lower-common-tangent ldi rdi)
            _ (println "lower common tangent (ldi rdi):" (edge-info ldi) (edge-info rdi))
            ;;
            ...
            _ (println "initial cross edge:" (edge-info initial-cross-edge))
          ...
          (loop [cross-edge initial-cross-edge]
            (println "cross-edge: " (edge-info cross-edge))
            (let [l-candidate (bubble-left! cross-edge)
                  r-candidate (bubble-right! cross-edge)
                  _ (println)
                  _ (println "candidates: l: " (edge-info l-candidate)
                             " r: " (edge-info r-candidate))
                  ;;
